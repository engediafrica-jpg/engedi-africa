...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      await fetchOrders(profileData, activeTab)
      setLoading(false)
    }
    getData()
  }, [])

  const fetchOrders = async (prof, tab) => {
    const field = tab === 'buying' ? 'buyer_id' : 'supplier_id'
    const { data } = await supabase
      .from('orders')
      .select('*, product:products(name, category, unit), buyer:profiles!orders_buyer_id_fkey(full_name, phone, city), supplier:profiles!orders_supplier_id_fkey(full_name, company_name, phone, city)')
      .eq(field, prof.id)
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    ...
    if (error) { setMessage('Error updating status'); setUpdating(null); return }
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
          {['buying', 'selling'].map(tab => (
            <button
              ...
              style={{ padding: '10px 24px', borderRadius: '8px', border: `1.5px solid ${activeTab === tab ? '#1A1A1A' : '#EEE6DA'}`, background: activeTab === tab ? '#1A1A1A' : '#FFFFFF', color: activeTab === tab ? '#FFFFFF' : '#666666', fontWeight: '700', fontSize: '14px', cursor: 'pointer', textTransform: 'capitalize' }}
            >
              {tab === 'buying' ? '🛒 My Orders' : '📦 Orders Received'}
            </button>
          ))}
        </div>
                    ...
                    <div style={{ background: '#F9F6F1', borderRadius: '10px', padding: '14px', marginBottom: '16px' }}>
                      ...
                      {order.delivery_address && (
                        <p style={{ margin: '6px 0 0', fontSize: '13px', color: '#666666' }}>
                          <strong style={{ color: '#1A1A1A' }}>Delivery address:</strong> {order.delivery_address}
                        </p>
                      )}
                      {order.buyer_note && (
                        <p style={{ margin: '6px 0 0', fontSize: '13px', color: '#666666' }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          {order.status === 'pending' && (
                            <button onClick={() => handleUpdateStatus(order.id, 'processing')} disabled={updating === order.id} style={{ background: '#6366F1', color: '#FFFFFF', border: 'none', padding: '10px 18px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                              Mark Processing
                            </button>
                          )}
                          {order.status === 'processing' && (
                            <button onClick={() => handleUpdateStatus(order.id, 'dispatched')} disabled={updating === order.id} style={{ background: '#F59E0B', color: '#FFFFFF', border: 'none', padding: '10px 18px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                              Mark Dispatched
                            </button>
                          )}
                          {order.status === 'dispatched' && (
                            <button onClick={() => handleUpdateStatus(order.id, 'delivered')} disabled={updating === order.id} style={{ background: '#2ecc71', color: '#FFFFFF', border: 'none', padding: '10px 18px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                              Mark Delivered
                            </button>
                          )}
                          <button onClick={() => handleUpdateStatus(order.id, 'cancelled')} disabled={updating === order.id} style={{ background: '#fde8e8', color: '#c0392b', border: '1px solid #c0392b', padding: '10px 18px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                            Cancel Order
                          </button>
                        </div>
                      </div>
