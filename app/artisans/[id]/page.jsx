...
function RequestQuoteButton({ profile, currentUser, supabase, router }) {
  ...
  if (done) return (
    <div style={{ background: '#1a2e1d', border: `1px solid ${C.oasis}44`, borderRadius: '10px', padding: '16px', textAlign: 'center' }}>
      ...
      <button onClick={() => router.push('/bookings')} style={{ background: 'none', border: 'none', color: C.clay, fontWeight: '600', cursor: 'pointer', fontSize: '13px' }}>
        View your bookings →
      </button>
    </div>
  )
  ...
  if (showForm) return (
    <div style={{ background: C.raised, border: `1px solid ${C.clay}44`, borderRadius: '12px', padding: '24px' }}>
      <h3 style={{ fontSize: '15px', fontWeight: '700', color: C.text, margin: '0 0 20px' }}>
        Request a Quote from {profile.full_name?.split(' ')[0]}
      </h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div>
          ...
          <input type="text" placeholder="e.g. Electrical wiring for 3-bedroom flat" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={inputStyle} />
        </div>
        <div>
          <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: C.muted, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Job Description *</label>
        ...
        <div>
          ...
          <input type="text" placeholder="Where is the job?" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} style={inputStyle} />
        </div>
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '140px' }}>
            ...
            <select value={form.scheduled_date} onChange={e => setForm({ ...form, scheduled_date: e.target.value })}
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={handleSubmit} disabled={submitting || !form.title || !form.description}
            style={{ flex: 2, background: submitting || !form.title || !form.description ? C.line : C.clay, color: submitting || !form.title || !form.description ? C.muted : C.sunk, border: 'none', padding: '12px', borderRadius: '8px', fontWeight: '700', fontSize: '14px', cursor: submitting || !form.title || !form.description ? 'not-allowed' : 'pointer' }}>
  ...
  return (
    <button onClick={() => setShowForm(true)}
      style={{ width: '100%', background: C.clay, color: C.sunk, border: 'none', padding: '16px', borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer' }}>
      Request a Quote
    </button>
  )
}
...
export default function ArtisanProfilePage({ params }) {
  ...
  const handleSubmitReview = async () => {
    ...
    if (error) { setReviewMessage('Error submitting'); setSubmittingReview(false); return }
    setReviews([data, ...reviews])
    setRating(0); setReviewBody(''); setShowReviewForm(false)
  ...
  const StarDisplay = ({ value, size = 18 }) => (
    <div style={{ display: 'flex', gap: '2px' }}>
      {[1,2,3,4,5].map(s => <span key={s} style={{ fontSize: size, color: s <= value ? '#F59E0B' : C.line }}>★</span>)}
    </div>
  )
    ...
    <div style={{ display: 'flex', gap: '4px', marginBottom: '16px' }}>
      {[1,2,3,4,5].map(s => (
        <span key={s} onClick={() => setRating(s)} onMouseEnter={() => setHoveredStar(s)} onMouseLeave={() => setHoveredStar(0)}
          style={{ fontSize: '32px', cursor: 'pointer', color: s <= (hoveredStar || rating) ? '#F59E0B' : C.line }}>★</span>
      ))}
    </div>
  )
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '12px', padding: '28px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <span style={{ background: C.sunk, border: `1px solid ${C.clay}44`, color: C.clay, fontSize: '12px', fontWeight: '600', padding: '2px 8px', borderRadius: '4px' }}>Artisan</span>
        ...
        <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {profile.company_name && <div style={{ display: 'flex', gap: '12px' }}><span style={{ fontSize: '13px', color: C.muted, minWidth: '120px' }}>Company</span><span style={{ fontSize: '13px', color: C.text, fontWeight: '600' }}>{profile.company_name}</span></div>}
            {profile.state && <div style={{ display: 'flex', gap: '12px' }}><span style={{ fontSize: '13px', color: C.muted, minWidth: '120px' }}>State</span><span style={{ fontSize: '13px', color: C.text, fontWeight: '600' }}>{profile.state}</span></div>}
            {profile.experience_years && <div style={{ display: 'flex', gap: '12px' }}><span style={{ fontSize: '13px', color: C.muted, minWidth: '120px' }}>Experience</span><span style={{ fontSize: '13px', color: C.text, fontWeight: '600' }}>{profile.experience_years} years</span></div>}
            <div style={{ display: 'flex', gap: '12px' }}><span style={{ fontSize: '13px', color: C.muted, minWidth: '120px' }}>Verification</span><span style={{ fontSize: '13px', color: profile.is_verified ? C.oasis : C.muted, fontWeight: '600' }}>{profile.is_verified ? '✓ Verified' : 'Not verified'}</span></div>
          </div>
        </div>
        ...
        <div style={{ background: C.raised, border: `1px solid ${C.line}`, borderRadius: '12px', padding: '24px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
            ...
            {currentUser?.id !== profile.id && !alreadyReviewed && (
              <button onClick={() => setShowReviewForm(!showReviewForm)}
                style={{ background: C.clay, color: C.sunk, border: 'none', padding: '8px 16px', borderRadius: '6px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
                + Write Review
              </button>
            )}
          </div>
          ...
          {showReviewForm && (
            <div style={{ background: C.sunk, borderRadius: '10px', padding: '20px', marginBottom: '16px' }}>
              <p style={{ fontSize: '13px', fontWeight: '700', color: C.text, margin: '0 0 12px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Your Rating</p>
              <StarInput />
              <div style={{ marginBottom: '14px' }}>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: C.muted, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Your review (optional)</label>
                <textarea value={reviewBody} onChange={e => setReviewBody(e.target.value)} rows={3} placeholder="Share your experience..."
                  style={{ ...inputStyle, resize: 'vertical' }} />
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={handleSubmitReview} disabled={submittingReview}
                  style={{ flex: 2, background: C.clay, color: C.sunk, border: 'none', padding: '10px', borderRadius: '8px', fontWeight: '700', fontSize: '13px', cursor: 'pointer' }}>
          ...
          {reviews.length === 0
            ? <p style={{ color: C.muted, fontSize: '14px', margin: 0 }}>No reviews yet. Be the first.</p>
            : reviews.map(review => (
