    ...
    <div className="mb-4 flex gap-1">
      {[1,2,3,4,5].map(s => (
        <span
          ...
          className={`cursor-pointer text-[32px] ${s <= (hoveredStar || rating) ? 'text-clay' : 'text-line-strong'}`}
        >★</span>
      ))}
    </div>
  )
      ...
      <div className="mx-auto max-w-[720px] px-6 py-10">
        ...
        <div className="ticks mb-6 border border-line bg-surface-raised p-8">
          <div className="flex flex-wrap items-start gap-5">
            ...
            <div className="flex-1">
              ...
              <Badge tone="pending">Artisan</Badge>
        ...
        <div className="mb-6 border border-line bg-surface-raised p-6">
          ...
          <div className="flex flex-col gap-3">
            {profile.company_name && <div className="flex gap-3"><span className="min-w-[120px] text-[13px] text-text-muted">Company</span><span className="text-[13px] font-semibold text-text">{profile.company_name}</span></div>}
            {profile.state && <div className="flex gap-3"><span className="min-w-[120px] text-[13px] text-text-muted">State</span><span className="text-[13px] font-semibold text-text">{profile.state}</span></div>}
            {profile.experience_years && <div className="flex gap-3"><span className="min-w-[120px] text-[13px] text-text-muted">Experience</span><span className="text-[13px] font-semibold text-text">{profile.experience_years} years</span></div>}
            <div className="flex gap-3"><span className="min-w-[120px] text-[13px] text-text-muted">Verification</span><span className={`text-[13px] font-semibold ${profile.is_verified ? 'text-oasis' : 'text-text-muted'}`}>{profile.is_verified ? 'Verified' : 'Not yet verified'}</span></div>
          </div>
        </div>
        ...
        <div className="mb-6 border border-line bg-surface-raised p-6">
          ...
          {currentUser?.id !== profile.id && !canReview && (
            <p className="mb-4 text-[12px] text-text-muted">You can leave a review once you&apos;ve completed a booking with this artisan.</p>
          )}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/55 p-6">
          <div className="ticks max-h-[90vh] w-full max-w-[480px] overflow-y-auto border border-line bg-surface-raised p-8">
            ...
            <div className="mb-3.5">
              <Input label="Job title" value={jobTitle} onChange={e => setJobTitle(e.target.value)} placeholder="e.g. Lay foundation blocks for 3-bedroom bungalow" />
            </div>
            <div className="mb-3.5">
