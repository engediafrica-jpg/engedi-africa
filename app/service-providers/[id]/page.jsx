...
function RequestQuoteButton({ profile, currentUser, supabase, router }) {
  ...
  if (done) return (
    <div style={{ background: '#1a2e1d', border: `1px solid ${C.oasis}44`, borderRadius: '10px', padding: '16px', textAlign: 'center' }}>
      <p style={{ margin: '0 0 6px', fontWeight: '700', color: C.oasis, fontSize: '14px' }}>✓ Request sent!</p>
  ...
  if (showForm) return (
    <div style={{ background: C.raised, border: `1px solid ${C.clay}44`, borderRadius: '12px', padding: '24px' }}>
      <h3 style={{ fontSize: '15px', fontWeight: '700', color: C.text, margin: '0 0 20px' }}>Request a Quote from {profile.company_name || profile.full_name?.split(' ')[0]}</h3>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div>
          <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: C.muted, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Service Title *</label>
          <input type="text" placeholder="e.g. Cleaning for 3-bedroom flat" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} style={inputStyle} />
        </div>
        <div>
          ...
          <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} placeholder="Describe what you need..." style={{ ...inputStyle, resize: 'vertical' }} />
        </div>
        <div>
          ...
          <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '8px' }}>
                <h1 style={{ fontSize: '20px', fontWeight: '800', color: C.text, margin: 0 }}>{profile.company_name || profile.full_name}</h1>
              ...
              <span style={{ background: C.sunk, border: `1px solid ${C.clay}44`, color: C.clay, fontSize: '12px', fontWeight: '600', padding: '2px 8px', borderRadius: '4px' }}>Service Provider</span>
        ...
        {currentUser?.id !== profile.id && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <button onClick={handleStartConversation} disabled={messaging || messageSent}
              ...
              {messageSent ? '✓ Chat started — redirecting...' : messaging ? 'Opening chat...' : `Message ${profile.company_name || profile.full_name?.split(' ')[0]}`}
            </button>
          </div>
        )}
      </div>
