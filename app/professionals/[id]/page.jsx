...
function RequestQuoteButton({ profile, currentUser, supabase, router, projectId, projectTitle }) {
  ...
  const availableLGAs = form.state ? (nigeriaLGAs[form.state] || []) : []
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginTop: projectId ? 0 : '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '140px' }}>
            ...
            <select value={form.state} onChange={e => handleStateChange(e.target.value)} style={{ ...inputStyle, appearance: 'none' }}>
          ...
          <div style={{ flex: 1, minWidth: '140px' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: C.muted, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>LGA / Area</label>
            <select value={form.lga} onChange={e => setForm({ ...form, lga: e.target.value })} disabled={!form.state}
              style={{ ...inputStyle, appearance: 'none', opacity: form.state ? 1 : 0.5, cursor: form.state ? 'pointer' : 'not-allowed' }}>
              <option value="" style={{ background: C.sunk }}>{form.state ? 'Select LGA' : 'Select a state first'}</option>
              {availableLGAs.map(lga => <option key={lga} value={lga} style={{ background: C.sunk }}>{lga}</option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: '140px' }}>
            ...
            <input type="date" min={today} value={form.scheduled_date} onChange={e => setForm({ ...form, scheduled_date: e.target.value })}
